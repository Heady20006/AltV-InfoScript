...
let InfoBoxes: Array<InfoBox> = [];

let convertToRadiants: boolean = false;
let speedInMP: boolean = false;
enum Alignement {
  Center = 0,
  Left = 1,
  Right = 2,
...
class InfoBox {
  ...
  Scale: number = 0.5;
  MarginLeft: number = Alignement.HorizontalLeft;
  MarginTop: number = Alignement.VerticalTop;
  Alignement: number = 1;
...
alt.everyTick(() => {
  ...
  if (alt.Player.local.vehicle) {
    let o = VehicleList.find(
      (p: { Hash: number }) =>
        p.Hash == game.getEntityModel(alt.Player.local.vehicle.scriptID)
    );
  ...
  if (choosenFlag !== 1) {
    if (result.isHit) {
      ...
      if (choosenFlag == 2) {
        let o = VehicleList.find(
          (p: { Hash: number }) => p.Hash == result.entityHash
        );
        o
    ...
    if (result.isHit) {
      ...
      rGroundHash.Title = `Name: ${
        TextureList.find(
          (t: { Hash: number }) => t.Hash == result.entityMaterial
        ).Name
      }`;
